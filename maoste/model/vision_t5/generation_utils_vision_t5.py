...
class GenerationMixin_VisionT5:
  ...
  def _prepare_input_ids_for_generation(self,
                                        bos_token_id: int) -> torch.LongTensor:
    if bos_token_id is None:
      raise ValueError(
          '`bos_token_id` has to be defined when no `input_ids` are provided.')
    return torch.ones(
        (1, 1), dtype=torch.long, device=self.device) * bos_token_id

  def _prepare_attention_mask_for_generation(
      self, input_ids: torch.Tensor, pad_token_id: int,
      eos_token_id: int) -> torch.LongTensor:
    is_pad_token_in_inputs_ids = (pad_token_id is not None) and (pad_token_id
                                                                 in input_ids)
  ...
  def _get_decoder_start_token_id(self,
    ...
    elif (hasattr(self.config, 'decoder') and
          hasattr(self.config.decoder, 'decoder_start_token_id') and
          self.config.decoder.decoder_start_token_id is not None):
      return self.config.decoder.decoder_start_token_id
    elif bos_token_id is not None:
      return bos_token_id
    elif (hasattr(self.config, 'decoder') and
          hasattr(self.config.decoder, 'bos_token_id') and
  ...
  @staticmethod
  def _reorder_cache(past: Tuple[torch.Tensor],
                     beam_idx: torch.Tensor) -> Tuple[torch.Tensor]:
    """
  ...
  def generate(self,
    ...
    bos_token_id = bos_token_id if bos_token_id is not None else self.config.bos_token_id
    eos_token_id = eos_token_id if eos_token_id is not None else self.config.eos_token_id
    vinvl_region_num = vinvl_region_num if vinvl_region_num is not None else self.config.vinvl_region_num
    ...
    if is_greedy_gen_mode:
      ...
      return self.greedy_search(
          ...
          eos_token_id=eos_token_id,
          vinvl_region_num=vinvl_region_num,
          **model_kwargs,
      )
      ...
      input_ids, model_kwargs = self._expand_inputs_for_generation(
          input_ids=input_ids,
          is_encoder_decoder=self.config.is_encoder_decoder,
          **model_kwargs)  #  expand_size=num_beams
  ...
  def get_vision_similarity_in_image(self, batch_token_ids: torch.Tensor,
                                     vision_similarity_matrix: torch.Tensor):
    mask_for_classifier_index = []
    cumsums = [0]
    max_length_batch = 0
    not_in_image_token_ids = self.config.not_in_image_token_ids
    in_image_token_ids = self.config.in_image_token_ids
    for token_ids in batch_token_ids:
      start = 0
      cur_mask = []
      cur_aspect_num = 0
      while start < len(token_ids):
        if token_ids[start] == not_in_image_token_ids[0]:
          cur_mask += [False] * len(not_in_image_token_ids)
          start += len(not_in_image_token_ids)
        elif token_ids[start:start +
                       len(in_image_token_ids)].tolist() == in_image_token_ids:
          cur_mask += [True] * len(in_image_token_ids)
          start += len(in_image_token_ids)
          cur_aspect_num += 1
        else:
          cur_mask.append(False)
          start += 1
      max_length_batch = max(max_length_batch, cur_aspect_num)
      mask_for_classifier_index.append(
          cur_mask[:vision_similarity_matrix.shape[1]])
      cumsums.append(cumsums[-1] + cur_aspect_num)
    mask_for_classifier_index = torch.tensor(mask_for_classifier_index)
    vis_simi = vision_similarity_matrix[mask_for_classifier_index]
    return vis_simi, cumsums, max_length_batch

  def _turn_vis_simi_to_pred(self, generate_token_ids,
                             vision_similarity_matrix: torch.Tensor):
    vsep_token_id = self.config.vsep_token_id
    vis_simi, cumsums, max_length_batch = self.get_vision_similarity_in_image(
        generate_token_ids, vision_similarity_matrix)
    vis_in_image_prob = vis_simi.view(
        -1, 3, self.config.vinvl_region_num).mean(1).argmax(1)
    batch_prob = torch.full((generate_token_ids.shape[0], max_length_batch + 1),
                            vsep_token_id)
    if len(vis_in_image_prob) == 0:
      return batch_prob.to(torch.long)
    for i, (st, ed) in enumerate(zip(cumsums[:-1], cumsums[1:])):
      batch_prob[i, 1:ed - st + 1] = vis_in_image_prob[st:ed]
    return batch_prob.to(torch.long)
  ...
  def greedy_search(self,
    ...
    vis_similarities_list = torch.full([batch_size, 1, vinvl_region_num],
                                       fill_value=0.0,
                                       dtype=torch.float32).to(input_ids.device)
    ...
    while cur_len < max_length:
      ...
      vis_similarities_list = torch.cat(
          [vis_similarities_list, outputs.vis_similarities], dim=-2)
    ...
    # vis_similarities_list into token
    # [bts, max_length, vis_box_num]
    batch_vis_pred = self._turn_vis_simi_to_pred(
        generate_token_ids=input_ids,
        vision_similarity_matrix=vis_similarities_list)
    batch_generate_token_ids = torch.cat(
        [input_ids, batch_vis_pred.to(input_ids.device)], dim=1)
    return batch_generate_token_ids
  ...
  def beam_search(self,
    ...
    decoded = beam_scorer.finalize(input_ids,
                                   beam_scores,
                                   next_tokens,
                                   next_indices,
                                   pad_token_id=pad_token_id,
...
def top_k_top_p_filtering(
  ...
  if top_k > 0:
    logits = TopKLogitsWarper(top_k=top_k,
                              ...
                              min_tokens_to_keep=min_tokens_to_keep)(None,
                                                                     logits)
